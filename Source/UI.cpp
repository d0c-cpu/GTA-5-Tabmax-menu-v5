...
namespace Mod_Hub_Base::UserInterface
{
	...
	*/
	...
	//	if (logend)
	//	{
	//		if (optionCount > 0 && !settings::movingmenu)
	//		{
	//			if (!smoothScroll)
	//				scrollCurrent = (float)settings::currentitletion;
	//			else {
	//				if (scrollCurrent > (float)settings::optionCount) Settings::scrolledOption = (float)Settings::optionCount;
	//				if (scrollCurrent >= (float)settings::currentitletion - getScrollSpeed(Settings::keyPressDelay2) && Settings::scrolledOption <= (float)Settings::currentitletion + getScrollSpeed(Settings::keyPressDelay2))Settings::scrolledOption = (float)Settings::currentitletion;
	//				if (scrollCurrent < (float)settings::currentitletion && Settings::scrolledOption < (float)Settings::optionCount)Settings::scrolledOption += (getScrollSpeed(Settings::keyPressDelay2) + getSpeedIncrease());
	//				if (scrollCurrent > (float)settings::currentitletion && Settings::scrolledOption > 1.0f) Settings::scrolledOption -= (getScrollSpeed(Settings::keyPressDelay2) + getSpeedIncrease());
	//			}
	//		}
	//	}
	//}
